...
async function apiRequest(path, payload) {
    ...
    if (!response.ok) {
        const message = data?.error?.message || 'No pudimos procesar la solicitud en este momento.';
...
async function startFlow() {
    ...
    switchView(2);
    document.getElementById('loading-title').innerText = 'Iniciando sesión de diagnóstico';
    document.getElementById('loading-sub').innerText = 'Conectando con el microservicio de IA...';
    ...
    try {
        ...
        const initialMessage = session.assistantMessage || 'Sesión iniciada. ¿Qué reto quieres priorizar primero?';
...
async function sendMessage() {
    ...
    try {
        ...
        hideTyping();
        const aiResponse = data.assistantMessage || 'He registrado tu respuesta para el diagnóstico.';
